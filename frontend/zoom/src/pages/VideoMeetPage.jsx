...
useEffect(() => {
  ...
  let handleEndCall = () => {
    ...
    // routeTo("/home");
    window.location.href = "/"; //ahiya pela /home hatu
  };
  ...
  // new changes for history
  const saveMeetingHistory = async () => {
    try {
      const roomCode = window.location.pathname.split("/").pop();

      await addToUserHistory(roomCode);

      console.log("Meeting saved:", roomCode);
    } catch (err) {
...
const getGridStyle = () => {
  ...
  if (isMobile) {
    ...
    if (totalParticipants === 2) {
      return {
        gridTemplateColumns: "1fr", // One video per row
    ...
    <div>
      {askForUsername === true ? (
        ...
        <div className={styles.meetVideoContainer}>
          ...
          {/*             that changes for make user to check split screen 
           <video
...
>
    ...
    <div className={styles.userTag}>You</div>
    ...
    />
            ...
            {videos.map((video) => (
              <div key={video.socketId} className={styles.videoBox}>
                <div className={styles.userTag}> {video.username || "Participant"}</div>
